...
contract('Bridge - [fee]', async(accounts) => {
    ...
    beforeEach(async() => {
        ...
        await BridgeInstance.adminChangeFeeHandler(feehandlerInstance.address);
    ...
    it('deposit passes if valid amount supplied', async() => {
        ...
        await feehandlerInstance.changeFee(1, 2, Ethers.utils.parseEther("0.5"))
        assert.equal(web3.utils.fromWei((await feehandlerInstance._fee.call(1, 2)), "ether"), "0.5");
    ...
    it('distribute fees', async() => {
        await feehandlerInstance.changeFee(1, 2, Ethers.utils.parseEther("1"), { from: relayer });
        assert.equal(web3.utils.fromWei((await feehandlerInstance._fee.call(1, 2)), "ether"), "1");

        // check the balance is 0
        assert.equal(web3.utils.fromWei((await web3.eth.getBalance(feehandlerInstance.address)), "ether"), "0");
        await BridgeInstance.deposit(destinationDomainID, resourceID, depositData, { value: Ethers.utils.parseEther("1") })
        assert.equal(web3.utils.fromWei((await web3.eth.getBalance(feehandlerInstance.address)), "ether"), "1");
        ...
        TruffleAssert.passes(
            await feehandlerInstance.transferFee(
                [accounts[1], accounts[2]], [payout, payout]
            )
        )
